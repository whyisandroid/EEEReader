...
public class AppServiceImpl implements AppService {
    ...
    public void login() throws BusinessException {
        ...
        if (BaseResp.SUCCESS.equals(resp.getStatus())) {
            ...
            if (EReaderApplication.getInstance().getLogin() != null && !EReaderApplication.getInstance().getLogin().getId().equals(resp.getData().getId())) {
                List<StoreBook> list = BookDBHelper.get(AppController.getController().getCurrentActivity()).queryAllBooks();
                for (int i = 0; i < list.size(); i++) {
                    BookDBHelper.get(AppController.getController().getCurrentActivity()).deleteBook(list.get(i));
                }

                DbUtils db = null;
                try {
                    db = DbUtils.create(AppController.getController().getCurrentActivity(), Constant.OUTPATH, Constant.DBNAME);
                    db.deleteAll(StoreBook.class);
                } catch (DbException e) {
...
//		nameValuePairs.add(new BasicNameValuePair("page", "20"));
    ...
    public void shelfRecommend(PageRq pageRq) throws BusinessException {
        ...
        nameValuePairs.add(new BasicNameValuePair("per_page", pageRq.getPer_page() + ""));
        request.addParameter(Request.AJAXPARAMS, nameValuePairs);
        request.setUrl(Config.HTTP_MY_SHELF_BOOKS_RECOMMEND);
        request.setR_calzz(BookResp.class);
        BookResp resp = EReaderApplication.getAppSocket().shortConnect(request);
        if (BaseResp.SUCCESS.equals(resp.getStatus())) {
            //
